...
用户在工作台确认首份标准简历后，宿主 Agent 继续运行：

```bash
ora-workbench profile-foundation
```

若返回常用资料缺口，引导用户进入工作台“个人档案 -> 补全常用资料”。姓名、手机号、
邮箱等基础项优先完成，高频建议项允许稍后补充；健康史、处分、犯罪记录、亲属回避和
家庭成员等上下文敏感项不会在首次建档时主动询问。身份证号、详细地址等敏感答案不要在
聊天中索取，工作台会把答案直接交给本机 Agent，产品云端不保存答案值。

首次常用资料补全是减少后续重复输入的建议流程，不会把招聘页面的无星选填项变成阻塞项。
